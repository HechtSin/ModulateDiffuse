#!/home/xing/Downloads/software/anaconda3/bin/python
import numpy as np
import phonopy
...
#phonon.set_modulations(dimension=[[ncell,0,0],[0,ncell,0],[0,0,1]],phonon_modes=[[[0.5,0.5,0],0,75,0]]) # q-point, mode starting from 0, amp, phase 
...
total_cell = read_vasp('MPOSCAR-202020_perfect')
...
write_vasp('POSCAR_interface',total_cell)
print ('moose')
...
Qxz = Qxz.reshape((ncell*8+1,ncell*8+1,2)) # the cell is 10 10 10
## scattering length
b_Br = 6.795 
b_Cs = 5.42
b_Pb = 9.405
...
four = np.rot90(four)
np.savetxt('CPB_four4.txt',four)
plt.imshow(np.log10(four),vmin=6.5,vmax=9.5,cmap='jet')
